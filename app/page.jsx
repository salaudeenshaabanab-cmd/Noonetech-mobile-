      ...
      <header style={{ borderBottom: "1px solid #E2E1DE", position: "sticky", top: 0, background: "#F5F5F4", zIndex: 20 }}>
        <div style={{ maxWidth: 1200, margin: "0 auto", padding: "16px 20px", display: "flex", alignItems: "center", gap: 16, flexWrap: "wrap" }}>
          <button className="btn" onClick={() => { setCategory("all"); setQuery(""); }} style={{ background: "none", padding: 0, display: "flex", alignItems: "center", gap: 10 }}>
            <img src="https://i.ibb.co/spZ35y4S/514-AA084-FDAE-423-F-9-AFE-532248-E27467.png" alt="NOONETECH" style={{ width: 34, height: 34, objectFit: "contain", flexShrink: 0 }} />
      ...
      <main style={{ maxWidth: 1200, margin: "0 auto", padding: "32px 20px 80px" }}>
        {loading ? (
          ...
          <>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(150px, 1fr))", gap: 1, background: "#E2E1DE", border: "1px solid #E2E1DE" }}>
              {filtered.map((p) => (
...
function ProductCard({ product, onAdd }) {
  ...
  return (
    <div className="prod-row" style={{ background: "#FFFFFF", padding: 10, display: "flex", flexDirection: "column" }}>
      <button className="btn" onClick={() => router.push(`/product/${product.id}`)} style={{ display: "block", width: "100%", padding: 0, background: "none", marginBottom: 8 }}>
        {product.imageUrl ? (
          <img src={product.imageUrl} alt={product.name} style={{ width: "100%", aspectRatio: "1 / 1", objectFit: "cover", display: "block" }} />
        ) : (
          <div style={{ width: "100%", aspectRatio: "1 / 1", background: `linear-gradient(160deg, ${hue}, #1E1B8F)` }} />
        )}
      </button>
      <button className="btn" onClick={() => router.push(`/product/${product.id}`)} style={{ display: "block", width: "100%", textAlign: "left", background: "none", padding: 0, marginBottom: 2 }}>
        <p style={{ margin: 0, fontSize: 12.5, fontWeight: 700, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{product.name}</p>
      </button>
      <p style={{ margin: "0 0 6px", fontSize: 10.5, color: "#8A8F98", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{product.spec}</p>
      {outOfStock ? (
        <p style={{ margin: "0 0 6px", fontSize: 10, color: "#B91C1C", fontWeight: 600 }}>Out of stock</p>
      ) : lowStock ? (
        <p style={{ margin: "0 0 6px", fontSize: 10, color: "#B45309", fontWeight: 600 }}>Only {product.stock} left</p>
      ) : null}
      <div style={{ marginTop: "auto", display: "flex", alignItems: "center", justifyContent: "space-between", gap: 6 }}>
        <span style={{ fontSize: 13.5, fontWeight: 700 }}>{currency(product.price)}</span>
        <button
          className="btn"
