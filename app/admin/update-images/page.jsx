...
export default function UpdateImagesPage() {
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      const res = await fetch(`/api/update-images?key=${encodeURIComponent(key)}`, {
        method: "POST",
  ...
  return (
    <div style={{ maxWidth: 600, margin: "0 auto", padding: 20, fontFamily: "Arial, sans-serif" }}>
      <h1 style={{ fontSize: 20, marginBottom: 16 }}>Update Product Images</h1>
      ...
      <label style={{ display: "block", fontSize: 13, marginBottom: 4 }}>Image updates JSON (array of name/spec/imageUrl)</label>
      <textarea
        value={json}
        onChange={(e) => setJson(e.target.value)}
        placeholder='[{"name": "iPhone 11", "spec": "64GB · UK Used", "imageUrl": "https://..."}]'
        rows={12}
