...
export default function UpdateGalleryPage() {
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      const res = await fetch(`/api/update-gallery?key=${encodeURIComponent(key)}`, {
        method: "POST",
  ...
  return (
    <div style={{ maxWidth: 600, margin: "0 auto", padding: 20, fontFamily: "Arial, sans-serif" }}>
      <h1 style={{ fontSize: 20, marginBottom: 16 }}>Update Product Image Gallery</h1>
      ...
      <label style={{ display: "block", fontSize: 13, marginBottom: 4 }}>Gallery updates JSON (array of name/images)</label>
      <textarea
        value={json}
        onChange={(e) => setJson(e.target.value)}
        placeholder='[{"name": "iPhone 12", "images": ["https://...", "https://..."]}]'
        rows={12}
